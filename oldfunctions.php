...
function parse_reports_data($reports_string) {
    if (empty($reports_string)) return [];
    ...
    foreach ($entries as $entry) {
        ...
        list($msg_id, $message, $report_data) = array_pad(explode(':', $entry, 3), 3, '');
        ...
        if (!isset($processed_messages[$msg_id])) {
            $processed_messages[$msg_id] = [
                'message_id' => intval($msg_id),
                'content' => wp_kses($message, []),
                'reporters' => []
            ];
        }
        ...
        foreach ($report_array as $reporter_id => $report_info) {
            ...
            $processed_messages[$msg_id]['reporters'][] = [
                'reporter_id' => $reporter_id,
                'reporter_name' => $reporter_name,
                'category' => isset($report_info['category']) ? 
                    wp_kses($report_info['category'], []) : 'Unknown Category',
    ...
    return array_values($processed_messages);
}
		...
		if ($active_tab === 'reports') {
			...
			<table class="wp-list-table widefat fixed striped">
				...
				foreach ($reports as $report) {
					...
					foreach ($report_data as $reporter_id => $data) {
						// Get reporter name
						$reporter_name = '';
						if ($reporter_id > 0) {
						...
						echo '<td><a href="#" class="user-details-link" data-user-id="' . esc_attr($report->reported_id) . 
							 '" data-user-name="' . esc_attr($report->reported_name) . 
							 '" data-user-ip="' . esc_attr($report->reported_ip) . '">' . 
						...
						echo '</div>';
						echo '</td>';
						echo '<td>' . wp_date('d/m/Y H:i', strtotime($report->report_date)) . '</td>';
                        echo '<td>';
						$category = $data['category'];
						$category_icons = [
							'spam' => '<span style="font-size: 1.2em; margin-right: 8px; vertical-align: middle;">⚠️</span>',
							'harassment' => '<span style="font-size: 1.2em; margin-right: 8px; vertical-align: middle;">🔪</span>',
							'offensive' => '<span style="font-size: 1.2em; margin-right: 8px; vertical-align: middle;">💣</span>',
							'inappropriate' => '<span style="font-size: 1.2em; margin-right: 8px; vertical-align: middle;">🔞</span>',
							'other' => '<span style="font-size: 1.2em; margin-right: 8px; vertical-align: middle;">❓</span>'
						];
						$icon = isset($category_icons[strtolower($category)]) ? $category_icons[strtolower($category)] : '';
						echo $icon . esc_html($category);
						echo '</td>';
                        echo '<td>' . esc_html($data['description']) . '</td>';
                        echo '<td>' . esc_html($reporter_id) . '</td>';
                        echo '<td>' . esc_html($reporter_name) . '</td>';
                        echo '</tr>';
                    }
                }
                ?>
            ...
            <table class="wp-list-table widefat fixed striped">
                ...
                foreach ($users as $user_data) {
                    ...
                    if (!empty($user_data->reports_data)) {
                        ...
                        // Count total reports across all messages
                        $total_reports = 0;
                        foreach ($reports as $report) {
...
jQuery(document).ready(function($) {
	...
	window.showReportPopup = function(element) {
		...
		$('#ip-lookup-content').html(content);
		$('#ip-lookup-popup').show();
	};
    ...
    function buildReportSummary(reports) {
        ...
        return `
            <div class="reports-summary">
                ...
                <p>Messages Reported: ${reports.length}</p>
            </div>
            <hr>
        `;
    }
        ...
        return reports.map((report, index) => `
            <div class="report-message-group">
                ...
                <div class="reporters-list">
                    <h5>Reports (${report.reporters.length}):</h5>
                    ${report.reporters.map(reporterData => buildReporterEntry(reporterData, reportedUser, report.message_id)).join('')}
                </div>
                
                <div class="message-actions">
                    <button class="button" onclick="deleteMessage(${report.message_id})">Delete Message</button>
                    ${reportedUser.ip ? `<button class="button" onclick="banIP('${reportedUser.ip}')">Ban IP</button>` : ''}
                </div>
            </div>
	...
	function buildReporterEntry(reporterData, reportedUser, messageId) {
		let reporterName = reporterData.reporter_name;
		if (reporterData.reporter_id < 0) {
			const rows = document.querySelectorAll('.wp-list-table tr');
			for (const row of rows) {
				if (row.cells[0].textContent.trim() === String(reporterData.reporter_id)) {
					reporterName = row.cells[1].textContent.trim();
					break;
				}
			}
		}
		...
		return `
			<div class="report-entry">
				<div class="report-timestamp">
					<strong>Time:</strong> ${reporterData.time}
				</div>
				
				<div class="users-info">
					<div class="reporter">
						<h4>Reported By:</h4> ${reporterName} (ID: ${reporterData.reporter_id})
					</div>
					<div class="reported">
						<h4>User Reported:</h4> ${reportedUser.name} (ID: ${reportedUser.id})
					</div>
				</div>
				
				<div class="report-details">
					<strong>Description:</strong> ${reporterData.description}
				</div>
				
				<div class="report-category">
					<strong>Category:</strong> ${reporterData.category}
				</div>
				
				<div class="action-buttons">
					<button class="button" onclick="deleteReport(${messageId}, ${reporterData.reporter_id})">
						Delete Report
					</button>
				</div>
			</div>
